...
public class ComboCommands {
    ...
    public static ResourceLocation initStandByCommand(LivingEntity a, Map<EnumSet<InputCommand>, ResourceLocation> map) {
        EnumSet<InputCommand> commands =
                a.getCapability(CapabilityInputState.INPUT_STATE).map((state)->state.getCommands(a)).orElseGet(()-> EnumSet.noneOf(InputCommand.class));
        ...
        return map.entrySet().stream()
                ...
                .orElseGet(ComboStateRegistry.NONE::getId);
    }
    
    public static void initDefaultStandByCommands() {
        DEAFULT_STANDBY.put(EnumSet.of(InputCommand.ON_GROUND, InputCommand.SNEAK, InputCommand.FORWARD, InputCommand.R_CLICK),
        ...
        DEAFULT_STANDBY.put(EnumSet.of(InputCommand.ON_GROUND, InputCommand.BACK, InputCommand.SNEAK, InputCommand.R_CLICK),
                ComboStateRegistry.UPPERSLASH.getId());
        DEAFULT_STANDBY.put(EnumSet.of(InputCommand.ON_GROUND, InputCommand.SNEAK, InputCommand.BACK, InputCommand.FORWARD, InputCommand.R_CLICK),
                ComboStateRegistry.VOID_SLASH.getId());
        DEAFULT_STANDBY.put(EnumSet.of(InputCommand.ON_GROUND, InputCommand.R_CLICK),
                ComboStateRegistry.COMBO_A1.getId());

        DEAFULT_STANDBY.put(EnumSet.of(InputCommand.ON_AIR, InputCommand.SNEAK, InputCommand.BACK, InputCommand.R_CLICK),
                ComboStateRegistry.AERIAL_CLEAVE.getId());
        DEAFULT_STANDBY.put(EnumSet.of(InputCommand.ON_AIR),
                ComboStateRegistry.AERIAL_RAVE_A1.getId());
    }
}
