...
public class PlacePreviewEntityRenderer extends EntityRenderer<PlacePreviewEntity> {
    ...
    @Override
    public void render(PlacePreviewEntity entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn,
            MultiBufferSource bufferIn, int packedLightIn) {
        BlockState blockstate = entityIn.getBlockState();
        if (blockstate.getRenderShape() == RenderShape.MODEL) {
            Level world = entityIn.getCommandSenderWorld();
            if (blockstate != world.getBlockState(entityIn.blockPosition())
                    && blockstate.getRenderShape() != RenderShape.INVISIBLE) {
                matrixStackIn.pushPose();
                Player player = Minecraft.getInstance().player;
                Vec3 pos = player.getLookAngle().add(0, 0.5, 0).scale(3.0)
                        .align(EnumSet.of(Direction.Axis.X, Direction.Axis.Y, Direction.Axis.Z));

                Vec3 offset = entityIn.position().subtract(player.position()).scale(-1)
                        .align(EnumSet.of(Direction.Axis.X, Direction.Axis.Y, Direction.Axis.Z));
                ...
                BlockPos blockpos = new BlockPos(
                        VectorHelper.f2i(entityIn.getX(), entityIn.getBoundingBox().maxY, entityIn.getZ()));
                matrixStackIn.translate(0.5D, 0.5D, 0.0D);
                BlockRenderDispatcher blockrendererdispatcher = Minecraft.getInstance().getBlockRenderer();
                /*
                 * Iterator var11 = RenderType.chunkBufferLayers().iterator();
                 * 
                 * while(var11.hasNext()) { RenderType type = (RenderType)var11.next(); if
                 * (RenderTypeLookup.canRenderInLayer(blockstate, type)) {
                 ...
                 * blockstate.getPositionRandom(entityIn.getPosition()),
                 ...
                 */
                {
                    ...
                    blockrendererdispatcher.getModelRenderer().tesselateBlock(world,
                            ...
                            bufferIn.getBuffer(type), false, RandomSource.create(),
                            blockstate.getSeed(entityIn.blockPosition()), OverlayTexture.NO_OVERLAY,
